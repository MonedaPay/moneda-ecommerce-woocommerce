	...
	private function process_webhook_data( AggregatedOrderStatusResponseInterface $payload ): bool {
		...
		switch ( $aggregated_status ) {
			...
			case AggregatedOrderStatus::OVERPAID:
				...
				$order->add_order_note( __( 'Payment completed via Ari10 Pay', 'moneda-ecommerce-for-woocommerce' ) );
				break;
			...
			case AggregatedOrderStatus::UNDERPAID:
				// Payment is pending.
				$order->update_status( 'on-hold', __( 'Payment pending via Ari10 Pay', 'moneda-ecommerce-for-woocommerce' ) );
				break;

			case AggregatedOrderStatus::CANCELLED:
			case AggregatedOrderStatus::FAILURE:
				// Payment failed or was cancelled.
				$order->update_status( 'failed', __( 'Payment failed or cancelled via Ari10 Pay', 'moneda-ecommerce-for-woocommerce' ) );
				break;
			default:
				// Unknown status.
				if ( 'yes' === $debug_enabled ) {
				...
				$order->add_order_note( sprintf( __( 'Received unknown payment status: %s', 'moneda-ecommerce-for-woocommerce' ), $status ?? 'null' ) );
				break;
		}
